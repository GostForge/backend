...
## Надежность Docker-старта (uploads)

- При запуске контейнера backend автоматически проверяет и при необходимости чинит права на `/app/uploads`.
- После проверки Java-процесс всегда запускается от непривилегированного пользователя `app`.
- Если основная директория хранилища недоступна для записи, backend автоматически переключается на аварийную директорию во временной ФС.

Опциональные настройки (Spring env):

- `STORAGE_LOCAL_ROOT` (по умолчанию: `uploads`)
- `STORAGE_LOCAL_ENABLE_EMERGENCY_FALLBACK` (по умолчанию: `true`)
- `STORAGE_LOCAL_EMERGENCY_ROOT` (по умолчанию: `${java.io.tmpdir}/gostforge-uploads`)
